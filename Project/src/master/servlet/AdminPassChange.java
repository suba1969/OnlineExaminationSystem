...
public class AdminPassChange extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ResultSet resultset = statement.executeQuery("select password from `admin` where password='" + cpassword+ "' and `admin_id`='" + session.getAttribute("id") + "'");
			if (resultset.next()) {
				pass = resultset.getString(1);
			}
			if (password.equals(confpass)) {
				if (pass.equals(cpassword)) {
					statement.executeUpdate("UPDATE `exam_portal`.`admin` SET password='"+password+"' WHERE(`admin_id`='"+ session.getAttribute("id") + "'); ");
					request.setAttribute("msg", "Successfully changed password!!!");
					request.getRequestDispatcher("addPassChange.jsp").forward(request, response);
					statement.close();
					cn.close();
				} else {
