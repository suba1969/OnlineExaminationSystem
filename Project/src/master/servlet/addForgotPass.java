...
/**
 * Servlet implementation class addForgotPass
 */
public class addForgotPass extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try{
			...
			PreparedStatement ps= cn.prepareStatement("UPDATE `exam_portal`.`admin` SET password=? WHERE(`admin_id`=?);");
			ps.setInt(2, uname);
			ps.setString(1, pass);
			int i=0;
        	i=ps.executeUpdate();
        	...
        	if(i>0 && pass.equals(confpass)) {
        		...
        		request.getRequestDispatcher("addforpass.jsp").forward(request, response);
